...
class Hirvio:
    ...
    def nollaa(self):
        self.x = 50  # lähtöpiste vas. reuna
        self.y = YLIN_VIIVA_Y         
        self.korjaa_y = 1
...
robotti = Robotti()
hirvio = Hirvio()
oliot = [nuotti, robotti, hirvio]


def main():
    for olio in oliot:
        olio.nollaa()   # luominen nollaa, mutta tämä tarvitaan uusintapeleissä
    pisteet = 0
    vaarin = False
    vauhti = 90   # ticks
    nuotti.arvo_nuotti()    
    nykyinen_indeksi = nuotti.arvottu_indeksi
    midi_play(nuotti.arvottu_indeksi, nykyinen_indeksi)  # soitettava ja sammutettava nuotti
    ...
    while True:
            ...
            if tapahtuma.type == pygame.QUIT:
                ...
                if tapahtuma.type == pygame.KEYDOWN:    
                    ...
                    if chr(tapahtuma.key) == nuotti.nykyinen_nimi():
                        ...
                        naytto.blit(teksti, (nuotti.x, nuotti.y + 50))    
                        naytto.blit(robotti.pic, (nuotti.x , robotti.y))  
                        pygame.display.flip()
                        ...
                        naytto.blit(teksti, (nuotti.x, nuotti.y + 50))     # JESS tai NO
                        naytto.blit(robotti.pic, (nuotti.x , robotti.y))  
                        pygame.display.flip()
                        pygame.time.delay(300) 
        ...
        if nuotti.y == hirvio.y:
            hirvio.korjaa_y = 0  # muutetaan hirvion liikerataa
