...
"""
    ...
    @points('10.pinta_alat_osa2')
    def test_6_luokka_kolmio_olemassa(self):
        try:
            from src.pinta_alat import SuorakulmainenKolmio
            a = SuorakulmainenKolmio(2,2)
        except Exception as e:
            self.fail(f'Konstruktorikutsu SuorakulmainenKolmio(2,2) antoi virheen \n{e}\n' + 
            'Varmista, että luokka on määritelty.')

    @points('10.pinta_alat_osa2')
    def test_7_kolmio_perinta(self):
        from src.pinta_alat import Suorakulmio, SuorakulmainenKolmio
        a = SuorakulmainenKolmio(2,2)
        self.assertTrue(isinstance(a, Suorakulmio), f"Luokan SuorakulmainenKolmio pitäisi " +
            'periä luokka Suorakulmio!')

    @points('10.pinta_alat_osa2')
    def test_8_kolmio_ei_uusia_attribuutteja(self):
        from src.pinta_alat import Suorakulmio, SuorakulmainenKolmio
        a = SuorakulmainenKolmio(2,2) 
        ref = reflect.Reflect()
        ref.set_object(a)
        attr = ref.list_attributes(True)
        self.assertTrue(len(attr) == 3, f"Luokalle SuorakulmainenKolmio ei saa määritellä " +
            "uusia attribuutteja!")

    @points('10.pinta_alat_osa2')
    def test_9_kolmio_str(self):
        from src.pinta_alat import Suorakulmio, SuorakulmainenKolmio
        test_cases = ((2,2), (3,3), (2,5), (5,4), (10,20))
        for test_case in test_cases:
            kolmio = SuorakulmainenKolmio(test_case[0], test_case[1])
            val = str(kolmio)
            corr = f"suorakulmainen kolmio {test_case[0]}x{test_case[1]}"
            self.assertEqual(corr, val, f'Metodin __str__ pitäisi palauttaa ' + 
                f'\n{corr}\nmutta nyt se palauttaa\n{val}\nkun olio on ' + 
                f'alustettu näin:\nSuorakulmainenKolmio({test_case[0]}, {test_case[1]})')
    
    @points('10.pinta_alat_osa2')
    def test_9b_kolmio_pinta_ala(self):
        from src.pinta_alat import Suorakulmio, SuorakulmainenKolmio
        test_cases = ((2,2), (3,3), (2,5), (5,4), (10,20))
        for test_case in test_cases:
            kolmio = SuorakulmainenKolmio(test_case[0], test_case[1])
            val = kolmio.pinta_ala()
            corr = test_case[0] * test_case[1] / 2
            self.assertAlmostEqual(corr, val, 2, f'Metodin pinta_ala pitäisi palauttaa ' + 
                f'\n{corr}\nmutta nyt se palauttaa\n{val}\nkun olio on ' + 
                f'alustettu näin:\nSuorakulmainenKolmio({test_case[0]}, {test_case[1]})')
